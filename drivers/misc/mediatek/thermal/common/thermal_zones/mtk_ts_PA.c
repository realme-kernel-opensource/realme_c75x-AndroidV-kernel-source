...
static __s32 mtkts_PA_4G_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
...
static __s32  mtk_ts_PA_4G_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__u64 dwVCriAP2 = 0;
...
static int mtkts_PA_change_mode(struct thermal_zone_device *thermal,
...
static struct thermal_zone_device_ops mtk_PA_4G_dev_ops = {
	...
	.change_mode = mtkts_PA_change_mode,
...
static const struct proc_ops mtkts_PA_4G_fops = {
	.proc_open = mtk_PA_4G_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtk_PA_4G_write,
	.proc_release = single_release,
};

static const struct proc_ops mtkts_PA_4G_param_fops = {
	.proc_open = mtkts_PA_4G_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_PA_4G_param_write,
	.proc_release = single_release,
};
...
static int mtk_PA_4G_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktzPA", trips, num_trip,
						NULL, &mtk_PA_4G_dev_ops,0, 0, 0,
						interval * 1000);

	return 0;
}

static void mtk_PA_unregister_thermal(void)
{
	mtkts_PA_4G_dprintk("[%s]\n", __func__);
...
static int mtkts_PA_probe(struct platform_device *pdev)
{
	int err = 0;
	int ret = 0;
	...
	thermistor_ch1_1 = iio_channel_get(&pdev->dev, "thermistor-ch3");
	...
	if (!mtkts_PA_4G_dir) {
		...
		entry = proc_create("tzPA", 0664, mtkts_PA_4G_dir,
				&mtkts_PA_4G_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("tzPA_param", 0664, mtkts_PA_4G_dir,
...
const struct of_device_id mt_thermistor_of_match_PA[2] = {
	{.compatible = "mediatek,mtboard-thermistor9", },
	{},
};
#endif

#define THERMAL_THERMISTOR_NAME    "mtboard-thermistor9"
static struct platform_driver mtk_thermal_PA_driver = {
	.remove = NULL,
	.shutdown = NULL,
	.probe = mtkts_PA_probe,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
		.name = THERMAL_THERMISTOR_NAME,
#if IS_ENABLED(CONFIG_OF)
		.of_match_table = mt_thermistor_of_match_PA,
#endif
	},
};

int mtk_PA_init(void)
{
        int i = 0;
	int err = 0;
	...
	err = platform_driver_register(&mtk_thermal_PA_driver);
	if (err) {
...
void mtk_PA_exit(void)
{
	mtkts_PA_4G_dprintk("[%s]\n", __func__);
	mtk_PA_unregister_thermal();
}
//module_init(mtk_PA_init);
//module_exit(mtk_PA_exit);
MODULE_LICENSE("GPL");
